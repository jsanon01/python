...
def web():
    print('\nweb analytics deals with user visits related to online ad campaigns.\n\nperformance monitoring deals with vertical or horizintal scaling.\n\nad targeting deals with search and display advertisements.\n\ncontent recommendation deals with user search patterns, ratings, and browser histories.'.title())

def finance():
    print("\nbanking and financial institutions can use 'credit risk modeling' to score\ncredit applications and predict if a borrower will default or not n the future.\n\nbank and financial institutions can leverage big data systems for 'detecting frauds'\nsuch as credit card frauds, money laundering, and insurance claim frauds.".title())

def healthcare():
    print('\nbig data can benefit the following healthcare applications:'.title())
    print('\n- epidemiological surveliance\t- patient similarity-based decision intelligence application\n- adverse drug events prediction\t- real=time healt monitoring\n- evidence-based medicine\t- detecting claim anomalities'.title())
...
def iot():
    print('iot refers to thing that are connected to the internet:')
